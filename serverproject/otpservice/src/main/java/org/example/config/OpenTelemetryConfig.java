    ...
    public OpenTelemetrySdk openTelemetry() {
        ...
        Resource resource = Resource.getDefault().merge(
                Resource.create(io.opentelemetry.api.common.Attributes.of(
                        ResourceAttributes.SERVICE_NAME, "otp-service"
                ))
        );
        ...
        System.out.println("✅ OpenTelemetry SDK initialized for service: otp-service");
        return sdk;
    }

    @Bean
    public Meter meter(OpenTelemetrySdk openTelemetry) {
        return openTelemetry.getMeter("otp-service");
    }

    @Bean
    public Tracer tracer(OpenTelemetrySdk openTelemetry) {
        return openTelemetry.getTracer("otp-service");
    }
